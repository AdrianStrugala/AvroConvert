...
namespace DUPA.Reflect
{
    ...
    public class ClassCache
    {
        ...
        public static void AddDefaultConverter<TAvro, TProperty>(Func<TAvro, Schema.Schema, TProperty> from, Func<TProperty, Schema.Schema, TAvro> to)
        {
        ...
        public IAvroFieldConverter GetDefaultConverter(Schema.Schema.Type tag, Type propType)
        {
            Type avroType;
            switch (tag)
            {
        ...
        public void LoadClassCache(Type objType, Schema.Schema s)
        {
            switch (s)
            {
                ...
                case NamedSchema ns:
                    ...
                    break;
                case UnionSchema us:
                    if (us.Schemas.Count == 2 && (us.Schemas[0].Tag == Schema.Schema.Type.Null || us.Schemas[1].Tag == Schema.Schema.Type.Null) && objType.IsClass)
                    {
                        // in this case objType will match the non null type in the union
                        foreach (var o in us.Schemas)
                        {
                            if (o.Tag != Schema.Schema.Type.Null)
                            {
                    ...
                    else
                    {
                        ...
                        foreach (var o in us.Schemas)
                        {
                            if (o.Tag == Schema.Schema.Type.Record && GetClass(o as RecordSchema) == null)
                            {
