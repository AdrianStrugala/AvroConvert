...
namespace DUPA.Specific
{
    ...
    public class SpecificDatumReader<T> : PreresolvingDatumReader<T>
    {
        ...
        public SpecificDatumReader(Schema.Schema writerSchema, Schema.Schema readerSchema) : base(writerSchema, readerSchema)
        {
        }

        /// <inheritdoc/>
        protected override bool IsReusable(Schema.Schema.Type tag)
        {
            switch (tag)
            {
        ...
        private class SpecificRecordAccess : RecordAccess
        {
            ...
            {
                return reuse ?? ObjectCreator.Instance.New(typeName, Schema.Schema.Type.Record);
            }

            public object GetField(object record, string fieldName, int fieldPos)
            {
                return record.GetType().GetProperty(fieldName)?.GetValue(record);
            ...
            {
                var field = record.GetType().GetProperty(fieldName);
        ...
        private class SpecificFixedAccess : FixedAccess
        {
            ...
            {
                return reuse ?? ObjectCreator.Instance.New(typeName, Schema.Schema.Type.Fixed);
            }
        ...
        private class SpecificArrayAccess : ArrayAccess
        {
            ...
            public object Create(object reuse)
            {
                ...
                if (reuse != null)
                {
                    ...
                    array = ObjectCreator.Instance.New(typeName, Schema.Schema.Type.Array) as IList;
                return array;
            }
        ...
        private class SpecificMapAccess : MapAccess
        {
            ...
            public object Create(object reuse)
            {
                ...
                if (reuse != null)
                {
                    ...
                    map = ObjectCreator.Instance.New(typeName, Schema.Schema.Type.Map) as System.Collections.IDictionary;
                return map;
            }
