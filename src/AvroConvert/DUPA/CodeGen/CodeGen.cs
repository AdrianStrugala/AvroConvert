...
namespace DUPA.CodeGen
{
    ...
    public class CodeGen
    {
        ...
        /// </summary>
        public IList<Schema.Schema> Schemas { get; private set; }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public CodeGen()
        {
            this.Schemas = new List<Schema.Schema>();
        ...
        public virtual void AddSchema(Schema.Schema schema)
        {
            Schemas.Add(schema);
        }
        ...
        {
            foreach (Schema.Schema schema in this.Schemas)
            {
                ...
                foreach (KeyValuePair<SchemaName, NamedSchema> sn in names)
                {
                    switch (sn.Value.Tag)
                    {
                        case Schema.Schema.Type.Enumeration: processEnum(sn.Value); break;
                        case Schema.Schema.Type.Fixed: processFixed(sn.Value); break;
                        case Schema.Schema.Type.Record: processRecord(sn.Value); break;
                        case Schema.Schema.Type.Error: processRecord(sn.Value); break;
                        default:
        ...
        {
            foreach (Protocol.Protocol protocol in Protocols)
            {
        ...
        protected virtual SchemaNames generateNames(Protocol.Protocol protocol)
        {
            ...
            foreach (Schema.Schema schema in protocol.Types)
        ...
        /// <returns></returns>
        protected virtual SchemaNames generateNames(Schema.Schema schema)
        {
            var names = new SchemaNames();
            addName(schema, names);
            return names;
        ...
        protected virtual void addName(Schema.Schema schema, SchemaNames names)
        {
            ...
            switch (schema.Tag)
            {
                ...
                case Schema.Schema.Type.Union:
                    ...
                    foreach (Schema.Schema usc in us.Schemas)
                        addName(usc, names);
                    break;
        ...
        protected virtual void processFixed(Schema.Schema schema)
        {
        ...
        protected virtual void processEnum(Schema.Schema schema)
        {
        ...
        protected virtual CodeTypeDeclaration processRecord(Schema.Schema schema)
        {
            ...
            bool isError = recordSchema.Tag == Schema.Schema.Type.Error;
        ...
        internal static string getType(Schema.Schema schema, bool nullible, ref bool nullibleEnum)
        {
            switch (schema.Tag)
            {
                case Schema.Schema.Type.Null:
                ...
                case Schema.Schema.Type.Union:
                    ...
                    Schema.Schema nullibleType = getNullableType(unionSchema);
        ...
        public static Schema.Schema getNullableType(UnionSchema schema)
        {
            Schema.Schema ret = null;
            if (schema.Count == 2)
            {
                bool nullable = false;
                foreach (Schema.Schema childSchema in schema.Schemas)
                {
                    if (childSchema.Tag == Schema.Schema.Type.Null)
                        nullable = true;
                    else
                        ret = childSchema;
                }
        ...
        /// generated property.
        /// </param>
        protected virtual void createSchemaField(Schema.Schema schema, CodeTypeDeclaration ctd, bool overrideFlag)
        {
            // create schema field
            ...
            var cmie = new CodeMethodInvokeExpression(
                new CodeMethodReferenceExpression(new CodeTypeReferenceExpression(typeof(Schema.Schema)), "Parse"),
                new CodeExpression[] { cpe });
