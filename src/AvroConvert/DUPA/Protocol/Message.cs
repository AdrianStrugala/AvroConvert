...
namespace DUPA.Protocol
{
    ...
    public class Message
    {
        ...
        /// </summary>
        public Schema.Schema Response { get; set; }
        ...
        /// listed.
        /// </param>
        public Message(string name, string doc, RecordSchema request, Schema.Schema response, UnionSchema error, bool? oneway)
        {
            ...
            if (error != null && error.CanRead(Schema.Schema.Parse("string")))
            {
                this.SupportedErrors = error;
            }
            else
            {
                this.SupportedErrors = (UnionSchema) Schema.Schema.Parse("[\"string\"]");
        ...
        internal static Message Parse(JProperty jmessage, SchemaNames names, string encspace)
        {
            ...
            PropertyMap props = Schema.Schema.GetProperties(jmessage.Value);
            RecordSchema schema = RecordSchema.NewInstance(Schema.Schema.Type.Record, jmessage.Value as JObject, props, names, encspace);

            JToken jresponse = jmessage.Value["response"];
            var response = Schema.Schema.ParseJson(jresponse, names, encspace);

            JToken jerrors = jmessage.Value["errors"];
            UnionSchema uerrorSchema = null;
            if (null != jerrors)
            {
                Schema.Schema errorSchema = Schema.Schema.ParseJson(jerrors, names, encspace);
