...
#pragma warning disable SA1600 // Elements should be documented
        ...
        public struct NameCtorKey : IEquatable<NameCtorKey>
        {
            ...
            public Schema.Schema.Type type { get; private set; }
            public NameCtorKey(string value1, Schema.Schema.Type value2)
                : this()
            {
                name = value1;
                type = value2;
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
            ...
            case Schema.Schema.Type.Union:
                {
                    ...
                    {
                        Schema.Schema s1 = unSchema.Schemas[0];
                        Schema.Schema s2 = unSchema.Schemas[1];

                        // Nullable ?
                        Type itemType = null;
                        if (s1.Tag == Schema.Schema.Type.Null)
                        {
                            itemType = GetType(s2);
                        }
                        else if (s2.Tag == Schema.Schema.Type.Null)
                        {
                            itemType = GetType(s1);
                        }
        ...
        /// No type found matching the given name.
        /// </exception>
        public Type GetType(string name, Schema.Schema.Type schemaType)
        {
            Type type = FindType(name);

            if (schemaType == Schema.Schema.Type.Map)
            {
                type = GenericMapType.MakeGenericType(typeof(string), type);
            }
            else if (schemaType == Schema.Schema.Type.Array)
            {
        ...
        /// No type found matching the given name.
        /// </exception>
        public object New(string name, Schema.Schema.Type schemaType)
        {
