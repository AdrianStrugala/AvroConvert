...
Winner in this category is official implemetation of Avro format in the C\# world: Apache.Avro. Unfortunetaly it doesn't support many of key types like: decimal, dictionary, list or Guid. That was the main reason why I decided to create AvroConvert.
...
\begin{enumerate}
	...
	\item Serialization with additional encoding - result contains header (with information about codec type) and encoded data. Decreases size of result, but increases serializaiton and deserializaiton time. Look at the AvroConvert Deflate on the benchmark screen.
	\item Headless serializaiton - result contains only data. Assumes, that schema is known upfront.
\end{enumerate}
