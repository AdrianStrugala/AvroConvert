...
namespace EhwarSoft.AvroConvert.Generic
{
    ...
    public abstract class PreresolvingDatumReader<T> : DatumReader<T>
    {
        ...
        // read a specific field from a IDecoder
        private delegate object IDecoderRead(IDecoder dec);
        // skip specific field(s) from a IDecoder
        private delegate void IDecoderSkip(IDecoder dec);
        // read & set fields on a record
        private delegate void FieldReader(object record, IDecoder IDecoder);
        ...
        public T Read(T reuse, IDecoder IDecoder)
        {
            return (T)_reader(reuse, IDecoder);
        }
        ...
        private object ReadRecord(object reuse, IDecoder IDecoder, RecordAccess recordAccess, IEnumerable<FieldReader> readSteps)
        {
        ...
        private object ReadMap(object reuse, IDecoder IDecoder, MapAccess mapAccess, ReadItem valueReader)
        {
            object map = mapAccess.Create(reuse);

            for (int n = (int)IDecoder.ReadMapStart(); n != 0; n = (int)IDecoder.ReadMapNext())
            {
                mapAccess.AddElements(map, n, valueReader, IDecoder, false);
            }
            return map;
        }
        ...
        private object ReadArray(object reuse, IDecoder IDecoder, ArrayAccess arrayAccess, ReadItem itemReader, bool itemReusable)
        {
            object array = arrayAccess.Create(reuse);
            int i = 0;
            for (int n = (int)IDecoder.ReadArrayStart(); n != 0; n = (int)IDecoder.ReadArrayNext())
            {
                arrayAccess.EnsureSize(ref array, i + n);
                arrayAccess.AddElements(array, n, i, itemReader, IDecoder, itemReusable);
                i += n;
            }
        ...
        private object ReadFixed(object reuse, IDecoder IDecoder, FixedAccess fixedAccess)
        {
            var fixedrec = fixedAccess.CreateFixed(reuse);
            IDecoder.ReadFixed(fixedAccess.GetFixedBuffer(fixedrec));
            return fixedrec;
        }
        ...
        private object ReadNull(object reuse, IDecoder IDecoder)
        {
            IDecoder.ReadNull();
            return null;
        }

        private object ReadBoolean(object reuse, IDecoder IDecoder)
        {
            return IDecoder.ReadBoolean();
        }

        private ReadItem Read(IDecoderRead IDecoderRead)
        {
            return (r, d) => IDecoderRead(d);
        }

        private IDecoderSkip GetSkip(Schema writerSchema)
        {
            switch (writerSchema.Tag)
            {
                case Schema.Type.Null:
                ...
                case Schema.Type.Error:
                case Schema.Type.Record:
                    var recordSkips = new List<IDecoderSkip>();
                ...
                case Schema.Type.Union:
                    ...
                    var lookup = new IDecoderSkip[unionSchema.Count];
        ...
        protected interface ArrayAccess
        {
            ...
            void AddElements(object array, int elements, int index, ReadItem itemReader, IDecoder IDecoder, bool reuse);
        }
        ...
        protected interface MapAccess
        {
            ...
            void AddElements(object map, int elements, ReadItem itemReader, IDecoder IDecoder, bool reuse);
        }
