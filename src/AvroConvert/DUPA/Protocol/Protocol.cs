...
namespace DUPA.Protocol
{
    ...
    public class Protocol
    {
        ...
        /// </summary>
        public IList<Schema.Schema> Types { get; set; }
        ...
        public Protocol(string name, string space,
                        string doc, IEnumerable<Schema.Schema> types,
                        IDictionary<string,Message> messages)
        {
            ...
            this.Doc = doc;
            this.Types = new List<Schema.Schema>(types);
        ...
        private static Protocol Parse(JToken jtok)
        {
            ...
            var types = new List<Schema.Schema>();
            if (jtypes is JArray)
            {
                foreach (JToken jtype in jtypes)
                {
                    var schema = Schema.Schema.ParseJson(jtype, names, space);
                    types.Add(schema);
                }
            }
        ...
        internal void WriteJson(Newtonsoft.Json.JsonTextWriter writer, SchemaNames names)
        {
            ...
            foreach (Schema.Schema type in this.Types)
        ...
        /// <returns></returns>
        private bool TypesEquals(IList<Schema.Schema> that)
        {
            if (Types.Count != that.Count) return false;
            foreach (Schema.Schema schema in Types)
